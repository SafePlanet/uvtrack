...
//				validateRequestDate(context.getRequestDateString(), userId);
...
//		return false;
	 ...
	 */
	private void validateRequestDate(String requestDateString, String userId) {
		...
		if (!(offset.isAfter(now.minusMinutes(config
			...
			LOG.error("Date in header is out of range: {}", requestDateString + " " +userId);
